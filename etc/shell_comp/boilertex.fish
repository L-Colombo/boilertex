...
function __fish_boilertex_global_optspecs
	string join \n h/help V/version
end

function __fish_boilertex_needs_command
	# Figure out if the current invocation already has a command.
	set -l cmd (commandline -opc)
	set -e cmd[1]
	argparse -s (__fish_boilertex_global_optspecs) -- $cmd 2>/dev/null
	or return
	if set -q argv[1]
...
function __fish_boilertex_using_subcommand
	set -l cmd (__fish_boilertex_needs_command)
	test -z "$cmd"
	and return 1
	contains -- $cmd[1] $argv
end
